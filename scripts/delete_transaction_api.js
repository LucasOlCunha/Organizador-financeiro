#!/usr/bin/env node
// Login, create a transaction, delete it, then verify it's gone
...
async function run() {
  try {
    ...
    console.log("Creating transaction to delete...");
    ...
    const delRes = await fetch(`${BASE}/transactions/${id}`, {
      method: "DELETE",
    ...
    const getRes = await fetch(`${BASE}/transactions/${id}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
